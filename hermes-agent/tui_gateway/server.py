...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        ...
        try:
            ...
            import re
            from cli import _AUDIO_EXTENSIONS as _AE
            _audio_pattern = re.compile(r'\[Attached files?:?\s*(.+?)\]|\[User attached file:\s*(.+?)\]', re.IGNORECASE)
            _audio_match = _audio_pattern.search(str(prompt))
            if _audio_match:
                _audio_path_str = (_audio_match.group(1) or _audio_match.group(2) or "").strip()
                if _audio_path_str:
                    from pathlib import Path as _P
                    _audio_p = _P(_audio_path_str.replace('"', '').replace("'", ""))
                    if _audio_p.is_file() and _audio_p.suffix.lower() in _AE:
                        try:
                            from tools.transcription_tools import transcribe_audio as _ta
                            _stt_r = _ta(str(_audio_p))
                            if _stt_r.get("success"):
                                _tx = _stt_r["transcript"].strip()
                                prompt = (
                                    f"[Audio transcription: {_audio_p.name}]\n{_tx}"
                                )
                                logger.info(
                                    "Auto-transcribed audio %s (%d chars) before agent",
                                    _audio_p.name,
                                    len(_tx),
                                )
                            else:
                                prompt = (
                                    f"[Audio file: {_audio_p.name} — STT failed: "
                                    f"{_stt_r.get('error', '?')}]"
                                )
                                logger.warning(
                                    "Auto-transcribe failed for %s: %s",
                                    _audio_p.name,
                                    _stt_r.get("error"),
                                )
                        except Exception as _stt_exc:
                            logger.warning(
                                "Auto-transcribe error for %s: %s",
                                _audio_p.name,
                                _stt_exc,
                            )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        is_audio = drop_path.suffix.lower() in _AUDIO_EXTENSIONS
        if is_audio:
            try:
                from tools.transcription_tools import transcribe_audio
                stt_result = transcribe_audio(str(drop_path))
                if stt_result.get("success"):
                    transcript_text = stt_result["transcript"].strip()
                    text = (
                        f"[Audio transcription: {drop_path.name}]\n"
                        f"{transcript_text}"
                    ) + (f"\n{remainder}" if remainder else "")
                    logger.info(
                        "Auto-transcribed audio %s (%s chars)",
                        drop_path.name,
                        len(transcript_text),
                    )
                else:
                    err_msg = stt_result.get("error", "unknown error")
                    text = (
                        f"[Audio file: {drop_path.name} — transcription failed: {err_msg}]"
                    ) + (f"\n{remainder}" if remainder else "")
                    logger.warning(
                        "Auto-transcribe failed for %s: %s",
                        drop_path.name,
                ...
                text = (
                    f"[Audio file: {drop_path.name} — STT not available: {exc}]"
                ) + (f"\n{remainder}" if remainder else "")
                logger.warning(
                    "Auto-transcribe error for %s: %s",
                    drop_path.name,
