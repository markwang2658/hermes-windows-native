...
class ShellFileOperations(FileOperations):
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 500) -> ReadResult:
        ...
        # ── Windows: Always use Python native I/O ────────────────
        # Git Bash (MSYS2) has path handling issues on Windows.
        # Use pure Python for all file reads on Windows platform.
        import platform
        if platform.system() == "Windows":
            return self._read_file_native_python(path, offset, limit)
    ...
    def _read_file_native_python(self, path: str, offset: int = 1, limit: int = 500) -> ReadResult:
        """
        Read file using Python native I/O (for Windows non-ASCII paths).
        
        Bypasses Git Bash shell commands which cannot handle Chinese/Unicode paths.
        Uses pure Python with UTF-8 encoding detection.
        """
        try:
            # Check file exists
            ...
            # Join back to string
            page_content = ''.join(paginated_lines)
            
            # Check for binary content
            if self._is_likely_binary(path, page_content[:1000] if page_content else ""):
                return ReadResult(
                    is_binary=True,
                    file_size=file_size,
            ...
            return ReadResult(
                content=self._add_line_numbers(page_content, offset),
                total_lines=total_lines,
                file_size=file_size,
                truncated=truncated,
                hint=hint
            ...
            return ReadResult(error=f"Failed to read file: {str(e)}")
