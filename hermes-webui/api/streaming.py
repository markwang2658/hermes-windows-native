    ...
    *,
    ...
    try:
        ...
        try:
            ...
            import re as _re
            _audio_pat = _re.compile(r'\[Attached files?:?\s*(.+?)\]|\[User attached file:\s*(.+?)\]', _re.IGNORECASE)
            _am = _audio_pat.search(str(msg_text) or "")
            if _am:
                _apath_s = (_am.group(1) or _am.group(2) or "").strip()
                if _apath_s:
                    from pathlib import Path as _P2
                    _ap = _P2(_apath_s.replace('"', '').replace("'", ""))
                    from cli import _AUDIO_EXTENSIONS as _AE2
                    if _ap.is_file() and _ap.suffix.lower() in _AE2:
                        try:
                            from tools.transcription_tools import transcribe_audio as _ta2
                            _sr = _ta2(str(_ap))
                            if _sr.get("success"):
                                _tx2 = _sr["transcript"].strip()
                                logger.info("[webui] Audio-only mode: transcribed %s (%d chars), skipping LLM call", _ap.name, len(_tx2))
                                put('token', {'delta': _tx2, 'finish': False})
                                _audio_user_msg = f'[Audio: {_ap.name}]'
                                put('done', {
                                    'session': {'session_id': session_id, 'messages': [
                                        {'role': 'user', 'content': msg_text or _audio_user_msg, 'timestamp': int(time.time())},
                                        {'role': 'assistant', 'content': _tx2, 'timestamp': int(time.time()), 'finish_reason': 'stop'},
                                    ]},
                                    'usage': {'input_tokens': 0, 'output_tokens': len(_tx2)},
                                })
                                with _agent_lock:
                                    ...
                                    s.context_messages = (_previous_context_messages or []) + [
                                        {'role': 'user', 'content': _audio_user_msg, 'timestamp': _now_ts},
                                        {'role': 'assistant', 'content': _tx2, 'timestamp': _now_ts, 'reasoning': None, 'finish_reason': 'stop'},
                                    ]
                                    _audio_display_msgs = [
                                        {'role': 'user', 'content': msg_text or _audio_user_msg, 'timestamp': _now_ts},
                                        {'role': 'assistant', 'content': _tx2, 'timestamp': _now_ts},
                                    ]
                                    s.messages = _merge_display_messages_after_agent_result(
                                        _previous_messages,
                                        _previous_context_messages,
                                        (_previous_context_messages or []) + _audio_display_msgs,
                                        msg_text,
                                    )
                                    logger.info("[webui] Audio-only mode: saved %d display messages", len(s.messages))
                                    s.save()
                                if _checkpoint_stop is not None:
                                    _checkpoint_stop.set()
                                return
                            else:
                                user_message = f"[Audio file: {_ap.name} — STT failed: {_sr.get('error','?')}]"
                                logger.warning("[webui] Auto-transcribe failed for %s", _ap.name)
                        except Exception as _sexc:
                            logger.warning("[webui] Auto-transcribe error for %s: %s", _ap.name, _sexc)
                    elif _ap.is_file():
                        logger.info("[webui] Matched [Attached files:] but not audio (%s), clearing audio flag for text/image mode", _ap.suffix)
                        _am = None

            # ── Image-only mode: local GLM-4.6V vision, skip K2.6 ──
            _has_image_only = False
            _image_path = None
            if attachments and not _am:
                for _att in (attachments or []):
                    if isinstance(_att, dict):
                        _att_mime = str(_att.get('mime') or '').strip()
                        _att_path = str(_att.get('path') or '').strip()
                        if _att_mime.startswith('image/') and _att_path:
                            _ip = Path(_att_path.replace('"', '').replace("'", ""))
                            if _ip.is_file():
                                _has_image_only = True
                                _image_path = str(_ip)
                                break
            if _has_image_only and _image_path:
                try:
                    from agent.auxiliary_client import _get_auxiliary_task_config as _get_vision_cfg
                    _vision_cfg = _get_vision_cfg('vision') or {}
                    _vision_timeout = float(_vision_cfg.get('timeout', 120) or 120)
                    _vision_temperature = float(_vision_cfg.get('temperature', 0.1) or 0.1)
                    _vision_max_tokens = int(_vision_cfg.get('max_tokens', 2000) or 2000)

                    import base64 as _b64
                    import mimetypes as _mt
                    from agent.auxiliary_client import call_llm as _call_vision, extract_content_or_reasoning as _extract_vision
                    _img_mime = _mt.guess_type(_image_path)[0] or 'image/png'
                    _img_b64 = _b64.b64encode(Path(_image_path).read_bytes()).decode('ascii')
                    _img_data_url = f"data:{_img_mime};base64,{_img_b64}"
                    _user_text_for_vision = (msg_text or "").strip() or "Please describe the content of this image"
                    _vision_messages = [{
                        "role": "user",
                        "content": [
                            {"type": "text", "text": _user_text_for_vision},
                            {"type": "image_url", "image_url": {"url": _img_data_url}},
                        ]
                    }]
                    logger.info("[webui] Vision-only mode: analyzing %s with local model", Path(_image_path).name)
                    _vision_resp = _call_vision(
                        task="vision",
                    ...
                    _vision_result = _extract_vision(_vision_resp) or "Vision recognition failed"
                    logger.info("[webui] Vision result: %d chars", len(_vision_result))
                    put('token', {'delta': _vision_result, 'finish': False})
                    _image_user_msg = f'[Image: {Path(_image_path).name}]'
                    put('done', {
                        'session': {'session_id': session_id, 'messages': [
                            {'role': 'user', 'content': msg_text or _image_user_msg, 'timestamp': int(time.time())},
                            {'role': 'assistant', 'content': _vision_result, 'timestamp': int(time.time()), 'finish_reason': 'stop'},
                        ]},
                        'usage': {'input_tokens': 0, 'output_tokens': len(_vision_result)},
                    })
                    with _agent_lock:
                        s.active_stream_id = None
                        s.pending_user_message = None
                        ...
                        _now_ts = int(time.time())
                        s.context_messages = (_previous_context_messages or []) + [
                            {'role': 'user', 'content': _image_user_msg, 'timestamp': _now_ts},
                            {'role': 'assistant', 'content': _vision_result, 'timestamp': _now_ts, 'reasoning': None, 'finish_reason': 'stop'},
                        ]
                        _image_display_msgs = [
                            {'role': 'user', 'content': msg_text or _image_user_msg, 'timestamp': _now_ts},
                            {'role': 'assistant', 'content': _vision_result, 'timestamp': _now_ts},
                        ]
                        s.messages = _merge_display_messages_after_agent_result(
                            _previous_messages,
                            _previous_context_messages,
                            (_previous_context_messages or []) + _image_display_msgs,
                            msg_text,
                        )
                        logger.info("[webui] Vision-only mode: saved %d display messages", len(s.messages))
                        s.save()
                    if _checkpoint_stop is not None:
                        _checkpoint_stop.set()
                    return
                except Exception as _vexc:
                    logger.warning("[webui] Vision-only mode failed: %s", _vexc)
                    user_message = f"[Vision failed: {_vexc}] {msg_text or ''}"

            # ── Text-only mode: local GLM-4.6V, skip K2.6 ──
            logger.info("[webui] Text-only check: attachments=%s, is_audio=%s, is_image=%s",
                        bool(attachments), bool(_am), _has_image_only)
            if attachments:
                for _dbg_idx, _dbg_att in enumerate(attachments):
                    logger.info("[webui]   attachment[%d]: %s", _dbg_idx, _dbg_att)

            _has_text_only = False
            _text_path = None
            if attachments and not _am and not _has_image_only:
                for _att in (attachments or []):
                    if isinstance(_att, dict):
                        _att_mime = str(_att.get('mime') or '').strip()
                        _att_path = str(_att.get('path') or '').strip()
                        _tp = Path(_att_path.replace('"', '').replace("'", ""))
                        if _tp.is_file() and (
                            _att_mime.startswith('text/') or
                            _tp.suffix.lower() in _TEXT_EXTENSIONS
                        ):
                            _has_text_only = True
                            _text_path = str(_tp)
                            break
            if _has_text_only and _text_path:
                try:
                    from agent.auxiliary_client import _get_auxiliary_task_config as _get_text_cfg
                    _text_cfg = _get_text_cfg('text') or {}
                    _text_max_chars = int(_text_cfg.get('max_file_chars', 50000) or 50000)
                    _text_timeout = float(_text_cfg.get('timeout', 120) or 120)
                    _text_temperature = float(_text_cfg.get('temperature', 0.5) or 0.5)
                    _text_max_tokens = int(_text_cfg.get('max_tokens', 8000) or 8000)

                    _encoding = 'utf-8'
                    try:
                        _file_content = Path(_text_path).read_text(encoding=_encoding)
                    except UnicodeDecodeError:
                        _encoding = 'gbk'
                        _file_content = Path(_text_path).read_text(encoding=_encoding)
                    if len(_file_content) > _text_max_chars:
                        _truncated_msg = f"\n\n[⚠️ File truncated, original length {len(_file_content)} chars, showing first {_text_max_chars} chars]"
                        _file_content = _file_content[:_text_max_chars] + _truncated_msg
                    logger.info("[webui] Text-only mode: reading %s (%d chars, encoding=%s)",
                                 Path(_text_path).name, len(_file_content), _encoding)
                    _user_question = (msg_text or "").strip() or "Please summarize the content of this file"
                    _text_messages = [{
                        "role": "user",
                        "content": f"{_user_question}\n\n---\n\nFile content:\n```\n{_file_content}\n```"
                    }]
                    from agent.auxiliary_client import call_llm as _call_text, extract_content_or_reasoning as _extract_text
                    logger.info("[webui] Text-only mode: processing %s with local text model", Path(_text_path).name)
                    _text_resp = _call_text(
                        task="text",
                        messages=_text_messages,
                        temperature=_text_temperature,
                        max_tokens=_text_max_tokens,
                        timeout=_text_timeout,
                    )
                    _text_result = _extract_text(_text_resp) or "File processing failed"
                    logger.info("[webui] Text result: %d chars", len(_text_result))
                    put('token', {'delta': _text_result, 'finish': False})
                    _text_user_msg = f'[Text: {Path(_text_path).name}]'
                    _text_asst_msg = _text_result
                    put('done', {
                        'session': {'session_id': session_id, 'messages': [
                            {'role': 'user', 'content': msg_text or _text_user_msg, 'timestamp': int(time.time())},
                            {'role': 'assistant', 'content': _text_asst_msg, 'timestamp': int(time.time()), 'finish_reason': 'stop'},
                        ]},
                        'usage': {'input_tokens': 0, 'output_tokens': len(_text_result)},
                    })
                    with _agent_lock:
                        s.active_stream_id = None
                        s.pending_user_message = None
                        ...
                        _now_ts = int(time.time())
                        s.context_messages = (_previous_context_messages or []) + [
                            {'role': 'user', 'content': _text_user_msg, 'timestamp': _now_ts},
                            {'role': 'assistant', 'content': _text_asst_msg, 'timestamp': _now_ts, 'reasoning': None, 'finish_reason': 'stop'},
                        ]
                        _text_display_msgs = [
                            {'role': 'user', 'content': msg_text or _text_user_msg, 'timestamp': _now_ts},
                            {'role': 'assistant', 'content': _text_asst_msg, 'timestamp': _now_ts},
                        ]
                        s.messages = _merge_display_messages_after_agent_result(
                            _previous_messages,
                            _previous_context_messages,
                            (_previous_context_messages or []) + _text_display_msgs,
                            msg_text,
                        )
                        logger.info("[webui] Text-only mode: saved %d display messages", len(s.messages))
                        s.save()
                        logger.info("[webui] Text-only mode: session saved to %s", s.path)
                    if _checkpoint_stop is not None:
                        _checkpoint_stop.set()
                    return
                except Exception as _texc:
                    logger.warning("[webui] Text-only mode failed for %s: %s", Path(_text_path).name, _texc)
                    user_message = f"[Text processing failed: {_texc}] {msg_text or ''}"
