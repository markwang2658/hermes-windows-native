...
def _replace_retry(src: Path, dst: Path, max_retries: int = 3, delay: float = 0.05) -> None:
    """Retry os.replace() on Windows to handle transient file locks (WinError 5).

    Faster-Whisper / antivirus / index writer may briefly lock the target file.
    """
    import errno
    for _attempt in range(max_retries):
        try:
            ...
            if _exc.errno == errno.EACCES and _attempt < max_retries - 1:
                time.sleep(delay * (_attempt + 1))
                continue
            raise
        ...
        if updates is None or not SESSION_INDEX_FILE.exists():
            ...
            try:
                with open(_tmp, 'w', encoding='utf-8') as f:
                    ...
                    f.flush()
                    os.fsync(f.fileno())
                _replace_retry(_tmp, SESSION_INDEX_FILE)
            except Exception:
